...
async function cargarConfiguracionSobres() {
    try {
        ...
        console.error('Error cargando sobres-config.json:', error);
        sobresConfigurados = [];
        bannersConfigurados = [];
    }
}
...
async function cargarEventos() {
    try {
        ...
        const config = await response.json();
        eventosConfigurados = config.banners || [];
    } catch (error) {
        console.warn('Error cargando eventos-config.json:', error);
    }
}

function obtenerSobresDisponibles() {
    return sobresConfigurados.length > 0 ? sobresConfigurados : [];
}
